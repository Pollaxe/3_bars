...
```
```python
input_coordinates() # Запрашивает ваши координаты и возвращает latitude, longitude.
```

```python
what_to_print(bars) # Запрашивает у пользователя какой бар нужно найти и выдает этот бар и его тип.
```

```python
print_bars(bar, bar_type) # По вводу бара, с указанием типа бара - выдает информацию в консоль.
```
